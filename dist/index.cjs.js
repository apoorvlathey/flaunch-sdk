...
const getAmountWithSlippage = (amount, slippage, swapType) => {
    ...
    const slippageMultiplier = swapType === "EXACT_IN"
        ? BigInt(1e18) - viem.parseEther(slippage)
        : BigInt(1e18) + viem.parseEther(slippage);
    return (absAmount * slippageMultiplier) / BigInt(1e18);
};
const ETH = viem.zeroAddress;
const ethToMemecoin = (params) => {
    ...
    if (params.swapType === "EXACT_IN") {
        ...
        path = [
            ...
            },
            {
                ...
                tickSpacing: 60,
                hooks: flaunchHooks,
                hookData: viem.encodeAbiParameters([{ type: "address", name: "referrer" }], [params.referrer ?? viem.zeroAddress]),
            },
        ];
        // Parameters for 'EXACT_IN' swap
        v4Params = viem.encodeAbiParameters(IV4RouterAbiExactInput, [
            {
                currencyIn: ETH,
                path: path,
        ...
        v4Params = viem.encodeAbiParameters(IV4RouterAbiExactOutput, [
            {
    ...
    const settleParams = viem.encodeAbiParameters([
        {
            type: "address",
            name: "currency",
        },
        ...
        ETH,
        params.swapType === "EXACT_IN"
            ? params.amountIn ?? viem.maxUint256
            : params.amountInMax ?? viem.maxUint256,
    ]);
    const takeParams = viem.encodeAbiParameters([
        {
            type: "address",
            name: "currency",
        },
        ...
        params.memecoin,
        params.swapType === "EXACT_IN"
            ? params.amountOutMin ?? viem.maxUint256
            : params.amountOut ?? viem.maxUint256,
    ]);
    // Encode router data
    const v4RouterData = viem.encodeAbiParameters([
    ...
    const sweepInput = viem.encodeAbiParameters([
    ...
    const urExecuteCalldata = viem.encodeFunctionData({
...
const memecoinToEthWithPermit2 = (params) => {
    ...
    const v4Actions = ("0x" +
        ...
        V4Actions.SETTLE_ALL +
        V4Actions.TAKE_ALL);
    const v4ExactInputParams = viem.encodeAbiParameters(IV4RouterAbiExactInput, [
        {
            ...
            path: [
                {
                    ...
                    tickSpacing: 60,
                    hooks: flaunchHooks,
                    hookData: viem.encodeAbiParameters([
                        {
                            type: "address",
                            name: "referrer",
                        },
                    ], [params.referrer ?? viem.zeroAddress]),
                },
                {
                    intermediateCurrency: ETH,
                    fee: 0,
    ...
    if (params.signature && params.permitSingle) {
        const urCommands = ("0x" +
            ...
            URCommands.V4_SWAP);
        const permit2PermitInput = viem.encodeAbiParameters([
            {
                type: "tuple",
                components: [
                    {
...
const getPermit2TypedData = ({ chainId, coinAddress, nonce, deadline, }) => {
    ...
    const message = {
        details: {
            token: coinAddress,
            amount: viem.maxUint160,
            expiration: deadline === undefined ? Number(viem.maxUint48) : Number(deadline),
            nonce,
        },
        spender: UniversalRouterAddress[chainId],
        sigDeadline: deadline === undefined ? viem.maxUint256 : deadline,
    };
    const typedData = {
...
class ReadPoolManager {
    ...
    poolStateSlot({ poolId }) {
        ...
        return viem.keccak256(viem.encodePacked(["bytes32", "bytes32"], [poolId, POOLS_SLOT]));
    }
    ...
    async positionInfo({ poolId, owner, tickLower, tickUpper, salt, }) {
        ...
        const positionKey = viem.keccak256(viem.encodePacked(["address", "int24", "int24", "bytes32"], [owner, tickLower, tickUpper, saltBytes32]));
        const stateSlot = this.poolStateSlot({ poolId });
        const POSITIONS_OFFSET = 6n;
        const positionMapping = uint256ToBytes32(bytes32ToUint256(stateSlot) + POSITIONS_OFFSET);
        const positionInfoSlot = viem.keccak256(viem.encodePacked(["bytes32", "bytes32"], [positionKey, positionMapping]));
        ...
        console.log({ data });
        const liquidity = viem.hexToBigInt(data[0]);
        const feeGrowthInside0LastX128 = viem.hexToBigInt(data[1]);
        const feeGrowthInside1LastX128 = viem.hexToBigInt(data[2]);
        return {
            liquidity,
            ...
            params: {
                ...
                path: [
                    ...
                    },
                    {
                        ...
                        intermediateCurrency: viem.zeroAddress,
                    },
                ],
            },
        });
...
class ReadWriteFlaunchSDK extends ReadFlaunchSDK {
    ...
    flaunchWithRevenueManager(params) {
        if (this.readWriteFlaunchZap.contract.address === viem.zeroAddress) {
